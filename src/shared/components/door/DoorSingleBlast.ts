...
export class SingleBlastDoor extends BaseDoor<DoorAttributes, DoorInstance> implements OnStart, Door {
	constructor(private doorService: DoorService) {
		super();
	}

	onMotorStep(value: number) {
		const newCF = this.doorService.getOffsetCFrame(this.originCFrame, this.attributes.offset, value);
